...
unsigned char pd_verified_state;
int get_usbpd_verifed_state(void)
{
	return pd_verified_state;
}
EXPORT_SYMBOL(get_usbpd_verifed_state);
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	pd_verified_state = adapter_dev->verifed ;
...
MODULE_DESCRIPTION("Xiaomi Adapter Class Device");
